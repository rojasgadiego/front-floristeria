    ...
    <template v-else-if="panel">
      ...
      <section v-if="alertas.length" class="seccion">
        <SectionHeader titulo="Notificaciones">
          ...
          <template #aside>
            <Badge :tono="hayUrgentes ? 'warn' : 'neutro'">
      ...
      <section class="seccion">
        ...
        <div class="grid-kpi">
            ...
            <!-- <template #pie>
              ...
              <template v-else>
                <span :class="variacionSemanal >= 0 ? 'sube' : 'baja'">
                  {{ Math.abs(variacionSemanal ?? 0).toFixed(1) }}%
                </span>
          ...
          <!-- <Kpi rotulo="Resultado del día" :valor="clp(hoyDia.utilidad)" :valor-rojo="hoyDia.utilidad < 0">
            <template #pie>
              Margen {{ Number(hoyDia.margen).toFixed(0) }}%
            </template>
          </Kpi>

          <Kpi rotulo="Caja" :valor="caja?.abiertaPor ? clp(caja.efectivo) : 'Cerrada'">
            <template #pie>
              <template v-if="caja?.abiertaPor">Abierta por {{ caja.abiertaPor }} · {{ hora(caja.abiertaEn) }}</template>
      ...
      <section class="seccion">
        <SectionHeader titulo="Lo que viene">
          ...
          <template #aside>
            <Badge tono="neutro">{{ eventos.length }} evento(s)</Badge>
          </template>
        </SectionHeader>

        <EmptyState v-if="!eventos.length" :icono="IconCalendar"
          texto="No hay eventos agendados para las próximas dos semanas." />
        <div v-else class="tarjetas">
          <div v-for="e in eventos" :key="e.referencia" class="evento" :class="{ 'evento--urgente': e.dias <= 2 }">
            <div class="evento__head">
              <b>{{ e.titulo }}</b>
              <Badge :tono="e.dias <= 2 ? 'warn' : 'neutro'">{{ textoDias(e.dias) }}</Badge>
            </div>
            <div class="evento__meta">{{ e.tipo }} · {{ fecha(e.fecha) }}</div>

            <div class="evento__plata">
              <div><span>Total</span><b class="dato">{{ clp(e.monto) }}</b></div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="seccion">
        ...
        <div v-else class="comparacion">
          <div class="comp-header">
            <span>Boletas</span>
            <span>{{ semanaPasada.boletas }}</span>
            <span class="comp-hoy">{{ hoyDia.boletas }}</span>
          </div>
          <div class="comp-fila">
            <span>Ingresos</span>
            <b class="dato">{{ clp(semanaPasada.vendido) }}</b>
            <span class="dato comp-hoy">{{ clp(hoyDia.vendido) }}</span>
          </div>
        </div>
      </section>
    </template>
...
.comp-fila {
  border-bottom: 1px dotted var(--border, #eee);
  font-size: .88rem;
  color: var(--text-soft);
}
